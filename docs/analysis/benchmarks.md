...
| eslint — 50 files, 100 problems | 1588 | 803 | **49%** |
| ruff — 30 errors | 349 | 157 | 55% |
...
- **lint now ~49–55%** (was ~1%). The compressor keeps each diagnostic's
  location + level + rule but drops the verbose human message and repeated file
  paths; full detail stays in the tee file on failure. Fixed in 0.1.8.
...
- Lint now saves ~50% (location + rule kept, prose dropped).
